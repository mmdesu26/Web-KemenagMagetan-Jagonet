...
const PrayerTimes = ({ currentDate, isMobile }) => {
  ...
  const updateCurrentPrayer = () => {
    ...
    for (let i = 0; i < prayerMinutes.length; i++) {
      if (currentTime >= prayerMinutes[i].minutes) {
        ...
        next = i === prayerMinutes.length - 1 ? prayerMinutes[0].name : prayerMinutes[i + 1].name;
        nextTime = i === prayerMinutes.length - 1 ? 
          prayerMinutes[0].minutes + 24 * 60 : prayerMinutes[i + 1].minutes;
      }
    }
    ...
    setNextPrayer(next);
    setTimeLeft(`${hoursLeft > 0 ? hoursLeft + ' jam ' : ''}${minutesLeft} menit`);
  };
 ...
 <motion.div
    ...
    >
      ...
      <div className={`grid ${isMobile ? 'grid-cols-3' : 'grid-cols-2'} gap-2 md:gap-3`}>
