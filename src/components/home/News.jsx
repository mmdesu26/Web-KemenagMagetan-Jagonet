...
const News = ({ category, title, isMobile }) => {
  ...
  return (
    ...
    >
      <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} justify-between items-start gap-4 mb-4 md:mb-6`}>
        <h2 className="text-xl md:text-2xl font-bold text-green-800">{title}</h2>
        ...
        <div className="flex items-center space-x-2 md:space-x-3">
          ...
          <a href={`/news?category=${category}`} className="text-green-600 hover:text-green-800 text-xs md:text-sm font-medium flex items-center">
      ...
      <div className="space-y-3 md:space-y-4">
        {filteredNews.slice(0, isMobile ? 2 : 3).map((item, index) => (
          <motion.article
            ...
            className={`flex ${isMobile ? 'flex-col' : 'flex-row'} gap-3 md:gap-4 p-3 md:p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors`}
          >
            <img
              src={item.image}
              alt={item.title}
              className={`${isMobile ? 'w-full h-40' : 'w-32 h-24 md:w-32 md:h-32'} object-cover rounded-lg`}
            />
            <div className="flex-1">
              ...
              <h3 className="font-bold text-sm md:text-lg mb-1 md:mb-2 text-green-800">{item.title}</h3>
              <a
                href={item.url}
