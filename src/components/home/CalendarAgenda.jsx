...
const CalendarAgenda = ({ currentDate, isMobile }) => {
  ...
  const renderDays = () => {
    ...
    for (let i = 1; i <= totalDays; i++) {
      ...
      const hasEvent = agenda.some(event => event.date.toDateString() === date.toDateString());
  ...
  const todayEvents = agenda.filter(event => 
 ...
 <motion.div
    ...
    >
      <div className="flex justify-between items-center mb-3 md:mb-4">
        ...
        <a href="/agenda" className="text-green-600 hover:text-green-800 text-xs md:text-sm font-medium flex items-center">
        ...
        {todayEvents.length > 0 ? (
          <div className="space-y-2 md:space-y-3">
            {todayEvents.slice(0, isMobile ? 1 : 3).map(event => (
              <div key={event.id} className="p-2 md:p-3 bg-gray-50 rounded-lg border border-gray-200 text-xs md:text-sm">
                <div className="font-bold text-green-800">{event.title}</div>
                <div className="text-gray-600 line-clamp-1">{event.description}</div>
                {!isMobile && (
                  <div className="text-xs mt-1 text-gray-500">{event.location}</div>
                )}
              </div>
            ))}
          </div>
